...
//+------------------------------------------------------------------+
IMTDataset* ReportColumn::DatasetFromArray(IMTReportAPI &api,MTAPIRES &res,const ReportColumn *columns,const UINT columns_total,CMTArrayBase &arr)
  {
//--- checks
...
//--- create dataset columns
   if((res=ReportColumnsAdd(*data,columns,columns_total))!=MT_RET_OK)
      return(nullptr);
//--- write rows
   for(UINT i=0,total=arr.Total();i<total;i++)
...
//+------------------------------------------------------------------+
MTAPIRES ReportColumn::ReportColumnsAdd(IMTDataset &data,const ReportColumn *columns,const UINT columns_total)
  {
//--- checks
...
//--- iterate columns
   ...
   for(UINT i=0;i<columns_total;i++)
     {
...
//+------------------------------------------------------------------+
MTAPIRES ReportColumn::ReportTablePrepare(IMTReportAPI &api,const ReportColumn *columns,const UINT columns_total)
  {
//--- checks
   if(!columns || !columns_total)
      return(MT_RET_ERR_PARAMS);
